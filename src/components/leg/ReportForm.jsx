...
const ReportForm = ({ report, onChange, densities }) => {
  ...
  // Get ordered list of all phases (operational + standby)
  const getAllPhasesOrdered = () => {
    const phases = [...operationalPhases];
    if (standbyPhase) phases.push(standbyPhase);
    return phases;
  };
  ...
  // Handle End value blur - check if we should carry over to next phase
  const handleEndBlur = (phaseId, equipmentKey, endValue) => {
    ...
    if (currentIndex >= 0 && currentIndex < allPhases.length - 1) {
      ...
      const nextStart = nextPhase.equipment[equipmentKey]?.start;

      if (!nextStart || nextStart === '') {
        // Empty - auto-fill silently
        const updatedNextPhase = {
          ...nextPhase,
          equipment: {
            ...nextPhase.equipment,
            [equipmentKey]: {
              ...nextPhase.equipment[equipmentKey],
              start: endValue,
            },
          },
        };
        handlePhaseChange(nextPhase.id, updatedNextPhase);
        toast.addToast(`${EQUIPMENT_LABELS[equipmentKey]} Start auto-filled`, 'info', 2000);
      } else if (nextStart !== endValue) {
        // Has different value - show confirmation modal
        setCarryOverModal({
          isOpen: true,
          equipmentKey,
          equipmentLabel: EQUIPMENT_LABELS[equipmentKey],
          prevEnd: endValue,
  ...
  // Confirm carry-over update
  const handleCarryOverConfirm = () => {
    const { targetPhaseId, equipmentKey, prevEnd } = carryOverModal;
    const targetPhase = report.phases.find(p => p.id === targetPhaseId);

    if (targetPhase) {
      const updatedPhase = {
        ...targetPhase,
        equipment: {
          ...targetPhase.equipment,
          [equipmentKey]: {
            ...targetPhase.equipment[equipmentKey],
            start: prevEnd,
          },
        },
      };
      handlePhaseChange(targetPhaseId, updatedPhase);
      toast.addToast(`${EQUIPMENT_LABELS[equipmentKey]} Start updated`, 'success', 2000);
    }
  };
  ...
  const handleAddPhase = () => {
    ...
    // Pre-fill Start values from last operational phase's End values
    if (operationalPhases.length > 0) {
      const lastPhase = operationalPhases[operationalPhases.length - 1];
      Object.keys(newPhase.equipment).forEach(key => {
        const lastEnd = lastPhase.equipment[key]?.end;
        if (lastEnd && lastEnd !== '') {
          newPhase.equipment[key].start = lastEnd;
        }
      });
    }
    ...
    onChange({ ...report, phases: newPhases });
    toast.addToast('Phase added with Start values from previous End', 'info', 3000);
  };
  ...
  return (
    <>
      <CounterCarryOverModal
        isOpen={carryOverModal.isOpen}
        onClose={() => setCarryOverModal({ ...carryOverModal, isOpen: false })}
        onConfirm={handleCarryOverConfirm}
        equipmentLabel={carryOverModal.equipmentLabel}
        prevEnd={carryOverModal.prevEnd}
        currentStart={carryOverModal.currentStart}
      />
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              <div>
                ...
                <input type="date" value={report.date}
                  ...
                  style={{ border: '1.5px solid #9fb3c8', background: '#fff' }}
                  className="w-full px-4 py-2.5 rounded-xl text-sm dark:bg-navy-800 dark:border-navy-600" />
              </div>
              <div>
                ...
                <input type="text" value={report.port}
                  ...
                  style={{ border: '1.5px solid #9fb3c8', background: '#fff' }}
                  className="w-full px-4 py-2.5 rounded-xl text-sm dark:bg-navy-800 dark:border-navy-600" placeholder="Singapore" />
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input type="time" value={report.timeEvents.sbe} step="360"
                      ...
                      style={{ border: '1.5px solid #9fb3c8', background: '#fff' }}
                      className="w-full px-3 py-2.5 rounded-xl text-sm font-mono dark:bg-navy-800 dark:border-navy-600" />
                  </div>
                  <div>
